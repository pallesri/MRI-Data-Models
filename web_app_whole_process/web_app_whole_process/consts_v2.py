...
CC =['chief complaint','chief complaints','subjective', 'reason for appointment', 'reason for visit',
    ...
    'reason forvisit', 'cc/reason for visit']
...
def readable_pdf_to_txt(file):
    ...
    with pdfplumber.open(file) as pdf:
        for page in pdf.pages:
            tmp = page.extract_text(x_tolerance=0.3)
            if tmp is not None:
                text.append(tmp)
    return '\n'.join(text)
...
def get_disease_symptom(sent_list):
...
# flair_models = [get_demo_date, get_demo_date, get_demo_date, get_demo_date]
flair_models = [get_anatomy, get_drug_chemical, get_procedure, get_disease_symptom]
        ...
        for i in range(len(header)):
            ...
            for j in range(ind[i], ind[i+1]):
                if len(tagged[p_flag][j]) > 0:
                    tmp.append(str(tagged[p_flag][j]))
            tmp.append('='*60)
        show.append('\n'.join(tmp))
    print('done')
    return show
