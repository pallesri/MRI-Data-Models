...
from consts_v2 import SAMPLE, split_by_header, pp_start, pp_end, header_lang, normal_lang, hp_list, match_list, readable_pdf_to_txt,get_demo_date,flair_models, get_ner_entity,get_ner_for_webapp
...
app.layout = html.Div(children=[
    ...
    html.Div(id='intermediate-value_2',
             style={'display': 'none'})
...
@app.callback(Output('intermediate-value_2', 'children'),
              [Input('upload-data', 'contents'),
               Input('trans-type', 'value'),
               Input('intermediate-value', 'children')])
def update_ner_intermediate(contents, folder, text):
    if contents is not None and folder == 'Upload other files':
        rawtxt = json.loads(text)['text']
        show = get_ner_for_webapp(rawtxt)
        return json.dumps({'0': show[0],
                    '1': show[1],
                    '2': show[2],
                    '3': show[3],
                    '4': show[4]})
...
@app.callback(Output('ner_view', 'children'),
              [Input('ner-tabs', 'value'),
               ...
               Input('doc-dropdown', 'value'),
               Input('trans-type', 'value'),
               Input('intermediate-value_2', 'children')])
def update_ner(tab, contents, value, folder, ner):
    p_flag = 0
    ...
    elif contents is not None and folder == 'Upload other files':
        # rawtxt = json.loads(text)['text']
        # show = get_ner_for_webapp(rawtxt)
        # print(show[0])
        show = json.loads(ner)
...
if __name__ == '__main__':
    #os.chdir(r'/Users/xuqinxin/PycharmProjects/web_app_whole_process')
